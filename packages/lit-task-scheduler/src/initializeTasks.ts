...
import { task as handleAutoTopUp, name as autoTopUpName } from 'lit-task-auto-top-up';
...
export default async function initializeTasks(taskClient: Agenda) {
  ...
  taskClient.define(AUTO_TOP_UP_TASK_NAME, async (job) => {
    ...
    // Otherwise we log successes and failures internally and exit without an error code
