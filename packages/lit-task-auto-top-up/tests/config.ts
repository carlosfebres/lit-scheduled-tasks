...
describe('Configuration', () => {
  describe('Required properties', () => {
    ...
    it('should require at least minter PK and faucet address to be provided', () => {
      expect(
        () => new Config(_.pick(COMPLETE_CONFIG, ['NFT_MINTER_ADDRESS', 'NFT_MINTER_KEY']))
      ).not.throw();
    });

    it('should fail if minter address is missing', () => {
      expect(() => new Config(_.omit(COMPLETE_CONFIG, ['NFT_MINTER_ADDRESS']))).throw();
    });

    it('should fail if minter key is missing', () => {
      expect(() => new Config(_.pick(COMPLETE_CONFIG, ['NFT_MINTER_KEY']))).throw();
    });
  });

  describe('RECIPIENT_LIST_URL', () => {
    it('should treat recipient list URL as optional', () => {
      expect(() => new Config(_.omit(COMPLETE_CONFIG, ['RECIPIENT_LIST_URL']))).not.throw();
    });

    it('should provide our default source URL when not defined in env', () => {
      const { config } = new Config(_.omit(COMPLETE_CONFIG, ['RECIPIENT_LIST_URL']));
